...
I think the result is relatively reliable. 

- Comment on the prediction; does it seem reasonable?
I think so. If you look into the original data, we can find that the surrounding data points show that cover sprays around .5. But, the prediction seems to be less significant.
...
loess uses moving average to fit a best curve(non-linear for most of the time), which means the input values are weighted.
- Comment on the fit; does it seem reasonable?
Not likely, the left tail and right tail is kind of weird, since the slope may not change that much.
- Comment on the prediction, including the SE.
The prediction is sound and the se is acceptable.
...
It has the most closest value to .5
- How would you know that a cubic fit is good enough?
with a low Root-mean-square deviation
