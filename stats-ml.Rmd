...
summary(m)
```
the p-value of variables are less, which means they are all usefull at any reasonable level.

```{r}
residual <- resid(m)
plot(residual)
```
the disperse of residual evenly distributed on both sides of the axis.
