...
My answers:
-The model seems not good to fit, since the residuals plot shows a pattern.
-The prediction seems reasonable, since the canopy cover is expected to be around 50% when distance equal to 0.5.
...
My answers:
-The difference between loess and glm is that loess is a non-parametric method that fits a smooth curve to the data, while glm is a parametric method that fits a linear model to the data.
-The fit seems not reasonable according to the head and tail part.
-The prediction looks better than the previous fit, since SE is smaller.
...
My answers:
-"randomForest" is an implementation of the random forest algorithm in R, a machine learning method used for classification and regression. The random forest algorithm builds multiple decision trees and aggregates their predictions to produce a final prediction.
-The fit seems reasonable, but it looks very rugged not very smooth line.
-The prediction seems reasonbale but higher than two previous fits.

The keyword here is "ensemble learning".
...
- How would you know that a cubic fit is good enough?
 ...
 My answer:
-It seems fit better, since the residual is smaller
-THe prediction looks good, since cover is 50% when distance equal to 0.5, but higher SE compared with previous results
-By checking R-square, RSE, p-value...
...
By checking cross-validation and residual plots.
...
By hypothesis test, we can assume that distance=0.5 using p value to decide whether it should be rejected or not.
- How would you incorporate `location` in your analyses? How would you know that it is meaningful to use it?
By fitting model with "location" variable, check p-value, R-square and other important value to determine.
