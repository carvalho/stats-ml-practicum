...
```{r}
# Logistic regression(Quasi binomial). The canopy cover decreases 
# by 0.508 units as distance increases by 1 unit each.
res <- resid(m)
plot(fitted(m),res)
abline(0,0)
# It seems that there is no pattern in residuals.
```
...
```{r}
# `loess` function fits a polynomial surface. The fit seems more reasonable since it increased at first then decreased. The SE also decreaed a little than the previous fit.
...
randomForest implements Breiman's random forest algorithm and used in unsupervised mode for assessing proximities among data points.

- Comment on the fit; how does it differ from the previous fits?
The fitted line has a sudden drop at the beginning, unlike previous fits.
...
We say the predictions are reliable when they could explain more variations.
...
Approaches that have fitted lines go through the point(0.5,0.5) would make the test easier to perform.

- How would you incorporate `location` in your analyses? How would you know that
it is meaningful to use it?
`Location` can be analyzed clearly if we plot the data by groups within `location`. If different groups have different patterns, we could know it's meaningful to analyze cover and distance by locations.
