...
The residuals on average close to zero and seems to have a pattern, so the fit is not very good. The variance if off raise some red flags.
The prediction seems reasonable but since the residual assumption does not hold, we should be cautious about the prediction results.
...
LOESS refers to the local regression. 

The fit is better than the previous linear model as the standard error is slightly smaller.
...
Use bootstrap to measure the random forest performance.
