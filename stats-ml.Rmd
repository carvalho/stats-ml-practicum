...
predict(m, data.frame(distance = 0.5,0.6,0.7,0.8), se = TRUE, type = "response")
...
## the residual plots shows that the model do not fit well as the residuals do not randomly located around 0.
## the prediction looks good as it shows small se.
...
predict(m, data.frame(distance = 0.5), se = TRUE)
plot(canopy$cover, canopy$cover- m$predicted)
```
...
The residual plot shows an increasing trend of fit and do not randomly located around 0.
- Comment on the prediction; how would you obtain a measure of uncertainty?
This prediction do not fit well.
...
- Comment on the fit and compare it to the first model; plot and check residuals.
  the residual plots are better compare with the first model as it is randomly distribute.
- Comment on the prediction and compare it to previous results.
The prediction shows a smaller se.
- How would you know that a cubic fit is good enough?
Since the se is very small.
...
##By checking the standard errors and the residual plots of the prediction.
...
##By putting a part of data into test data and another part of the data into training to see if the test data satisfy this situation.
- How would you incorporate `location` in your analyses? How would you know that
it is meaningful to use it?
## by adding it into the model. We can checl the p-values and residuals plot to see whether adding this variable can help to make a better prediction.