...
The p value less than 0.05, the model fit the data well. Long Tail Distribution
- Comment on the prediction; does it seem reasonable?
From the plot, the value should be 0.05, so it's not reasonable.
## ML 1: LOESS
...
if relationship between variables is linear should use glm,If the relationship were nonlinear but smooth, you could use loess. The loess method is based on locally linear smoothing and can handle outliers.

- Comment on the fit; does it seem reasonable?
I don't think it fit

- Comment on the prediction, including the SE.
The value of prediction near to 0.5.
...
Random Forest is a powerful and versatile supervised machine learning algorithm that grows and combines multiple decision trees to create a “forest.”It builds decision trees on different samples and takes their majority vote for classification and average in case of regression.


- Comment on the fit; how does it differ from the previous fits?
I don't think it fits well, when the distance is 1, the value of cover is not 1,but when it depart from distance 1, the value of cover near to 1

- Comment on the prediction; how would you obtain a measure of uncertainty?
The value is 0.53, larger than 0.5, so it is not reasonable.
...
- Comment on the fit and compare it to the first model; plot and check residuals.
 I think the model fit the data well.
- Comment on the prediction and compare it to previous results.
the value is very near to the 0.5, so it's good
- How would you know that a cubic fit is good enough?
From residual plot and QQplot. 
...
I compare them to 0.5 because it is when distance is 0.5 that cover should be 0.5
...
I think we can test the model from residualvs fitted plot, QQplot, residual vs leverage and scale-location plot.

- How would you incorporate `location` in your analyses? How would you know that
it is meaningful to use it?

I think location could be used for classify the group, find differences between groups and then fit model to get predictions in each group.
