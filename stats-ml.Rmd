...
Answers:
- According to the residuals plot, the residuals are around 0, which means the model fits well; the second plot shows that the residuals should be normally distributed.
- The standard error of the prediction shows that the prediction accuracy is good
...
Answers:
- The loess function is a method for fitting a smooth curve to a scatterplot. It helps to remove the noise from the data.
- The fit is inappropriate in the head and tail part, according to the fitting plot.
- The SE of the prediction shows that it's more accurate than the linear fit model above
...
Answers
- `randomForest` s based on the concept of decision trees.Each decision tree in the forest is created using a random subset of the training data, and the prediction is made by combining the results from each tree. The final prediction is based on a majority vote or an average of the results from each tree.
- This fit model  overcome overfitting and bias.It provides a robust model that can handle complex data distributions and also improve accuracy by reducing variance in the predictions.
- The plot shows that the estimated error become stable when the "trees" grow bigger. The plot also shows that the model performs well in prediction since the error is small.
...
Answers:
- The residual plot shows that the residuals are around 0, there is linear relationship between predictors and response.
The second plot shows that the residuals are normally distributed.1
- This prediction is even better than the models above, the standard error is relatively small.
- Can evaluate according to R square, RMSE and so on.
...
Answer: Base on the accuracy, confidence interval, robustness and so on
