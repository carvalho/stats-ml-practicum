...
plot(m)
```
The residuals plot show that is not normal distribution.I am not sure it follow the quasibinomial well.
The prediction looks good, but it looks more accurate in moddle part.
...
?loess
```
It used to fit polynomal surface model.
Form the graph, it is reasonable.
The prediction looks well, but still have question in the begining and end part.
## ML 2: Random Forest
...
We can check the estimate value and test value.If it fit well, we can say that reliable.
